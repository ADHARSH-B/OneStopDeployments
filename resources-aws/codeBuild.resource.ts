...
enum codeBuildDefaultValues {
  ...
  codeBuildArtifactsLocation="mlrepodsd121",
  // artifactsLocations="",
...
export const createCodebuild = async(codeBuildProjectSpec:codeBuildDataType.codeBuildParams)=>{
    ...
    const codeBuildParams:any = { 
        ...
        artifacts: { 
          ...
          location: codeBuildProjectSpec?.artifacts?.location ?? (await createS3Bucket({Bucket:"sds1212asd"})).Location?.split("//")[1].split(".")[0],
        },
        environment: { 
